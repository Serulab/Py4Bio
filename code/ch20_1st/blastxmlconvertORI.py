...
or multiple HTML files. This version requires Biopython 1.45 
...
parser.add_option("-o", "--output", dest="o_file", default=None,
...
if len(args)<1:
    ...
    f = args[0]
    if opts.o_file is None:
        opts.o_file = f[:-3]+'html'
    doconvert(f, opts.o_file, opts.desc_n, opts.align_n)
elif len(args)>1:
    for f in args:
        outfile = f[:-3]+'html'
        doconvert(f, outfile, opts.desc_n, opts.align_n)