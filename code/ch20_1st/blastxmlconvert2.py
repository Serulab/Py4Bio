...
def html_head(f_in):
    ...
    return {'version_date': version_date, 'application':
            application, 'reference': reference}

def html_foot(f_in):
    tree = cET.parse(f_in)
    root = tree.getroot()
    db_version = root.findtext('BlastOutput_db')
    ...
    return {'db_version': db_version,
            'num_letter_db':num_letter_db,
            'num_seqs_db': num_seqs_db,
            'lambd': float(lambd),
            'kappa': float(kappa),
            'entrop': float(entrop),
            'b_prg': b_prg,
            'p_sc_match': p_sc_match,
            'p_sc_mismatch': p_sc_mismatch,
            'p_gap_open': p_gap_open,
            'p_gap_extend': p_gap_extend,
            'num_seqs_db': num_seqs_db,
            'num_letter_db': num_letter_db
            }
...
def do_convert(f_in, outfile, desc, align):
    out_d = htmlhead(f_in)
    out_d.update(blastconv(f_in, desc, align))
    out_d.update(htmlfoot(f_in))
    # send HTML
    return None
